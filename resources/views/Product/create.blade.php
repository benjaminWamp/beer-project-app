...
<div class="mt-4">
    <form action="{{ route('product.store') }}" method="post" enctype="multipart/form-data"> {{--enctype for enabled file post--}}
        ...
        <div class="mb-4">
            <label for="category_id" class="font-title block text-m font-semibold leading-6 text-gray-900">Catégorie</label>
                @error("category_id")
            <div class="text-red-500">{{$message}}</div>
                @enderror
            <div class="mt-2.5">
                <select  name="category_id" id="category_id" placeholder="Selectionnez une catégorie" class="block border w-full rounded-md border-0 px-3.5 py-2 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6">
