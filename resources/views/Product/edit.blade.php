...
<div class="mt-2 px-6">
    ...
    <form action="{{ route('product.update', $product) }}" method="post" enctype="multipart/form-data"> {{--enctype for enabled file post--}}
        ...
        <div class="mb-4">
            <label for="categories" class="font-title block text-m font-semibold leading-6 text-gray-900">Catégorie</label>
            @error("categories")
            <div class="text-red-500">{{$message}}</div>
            @enderror
            <div class="mt-2.5">
        ...
        <div class="mb-4">
            ...
            @enderror
            <div class="mt-2.5">    
                <input type="number" min="0" step="0.01" name="price_ht" id="price_ht" value="{{$product->price_ht}}" class="block border w-full rounded-md border-0 px-3.5 py-2 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6">
            </div>
        </div>
        <div class="mb-4">
